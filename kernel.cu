...
const int BLOCK_COUNT = 4096;  // 226 <-- UNSTABLE limit (not enough memory on the stack)
...
__device__ U32 outputBufferDevice[OUTPUT_BUFFER_LEN * sizeof(U32)] = { 0 };

__global__ void l2aKernel(const U8* __restrict__ initialKey) {
    ...
    */
    ...
    for (int j = 0; j < ITERATIONS_PER_INVOCATION; j++) {
        ...
        */
        ...
        for (int i = ACTUAL_DATA_SIZE - 1; i > ACTUAL_DATA_SIZE - L2A_SAFE_CHARS_COUNT; i--) {
            ...
            data_index = (data_index + initialKey[(ACTUAL_DATA_SIZE - i) % KEY_LEN]) % i;
    ...
    outputBufferDevice[invocation_id] = result;
}
...
void l2aWithCuda(const U8* __restrict__ initialKey) {
    U8* initialKeyDevicePtr = 0;
    // Select GPU
    ...
    // Launch kernel
    l2aKernel<<<BLOCK_COUNT, THREADS_COUNT>>> (initialKeyDevicePtr);
...
// TODO: Try nvprof for more accurate benchamrks
int main() {
    const U8 INITIAL_KEY[KEY_LEN] = { 24, 4, 25, 15, 25, 15, 25 };
    // Allocating output buffer on unified memory
    U32* outputBufferHost = (U32*) malloc(OUTPUT_BUFFER_LEN * sizeof(U32));
    ...
    // Running CUDA kernel
    l2aWithCuda(INITIAL_KEY);
    cudaUnwrap(cudaMemcpyFromSymbol(outputBufferHost, outputBufferDevice, OUTPUT_BUFFER_LEN * sizeof(U32), 0, cudaMemcpyDeviceToHost));
